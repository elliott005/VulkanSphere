...
public:
    ...
    void createTextureImage(const char* filename, Image* image) {
    ...
    void destroyBuffers(VkBuffer& vertexBuffer, VkDeviceMemory vertexBufferMemory, VkBuffer& indexBuffer, VkDeviceMemory indexBufferMemory) {
    ...
    void updateVertexBuffer(VkBuffer& buffer, std::vector<Vertex> vertices) {
        VkDeviceSize bufferSize = sizeof(vertices[0]) * vertices.size();
...
private:
    ...
    void initVulkan(const std::string vertexShader, const std::string fragmentShader) {
        createInstance();
        setupDebugMessenger();
        createSurface();
        pickPhysicalDevice();
    ...
    void createTextureSampler(Image* image) {
    ...
    void createDescriptorSets(std::vector<Image*> &images) {
    ...
    void drawFrame(VkBuffer& vertexBuffer, VkBuffer& indexBuffer, std::vector<uint32_t> indices, UniformBufferObject ubo) {
        ...
        recordCommandBuffer(commandBuffers[currentFrame], imageIndex, vertexBuffer, indexBuffer, indices);
